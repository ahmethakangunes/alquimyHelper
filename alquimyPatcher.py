...
class Patcher:
    ...
    def downloadFile(self, programName):
        ...
        response = requests.post('http://xxxxxxx:xxxxxx/patcher', data=json.dumps(self.data), headers=self.headers, timeout=60, stream=True)
    ...
    def run(self):
        response = requests.post('http://xxxxxxx:xxxxxx/patcher', data=json.dumps(self.data), headers=self.headers, timeout=10).json()
