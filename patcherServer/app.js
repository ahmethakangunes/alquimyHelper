...
const verifyShaMiddleware = (req, res, next) => {
    const expectedSha = 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx';
...
app.post('/patcher', (req, res) => {
    if (uploadStatus) {
        ...
        const result = Object.values(pcIps).includes(remoteAddress) ? 1 : 0;
    
        if (version != '2.1') {
            res.json({status: 203, message: 'Please use the updated version'});
        } else if (!result) {
            res.json({status: 205, message: 'WRONG IP ADRESS'});
        } else {
            if (username == 'xxxx' && password == 'xxxx') {
                if (!downloadProgramName) {
                    ...
                    const folderPath = '/root/patcher/programs';
                    ...
                    const filePath = path.join('/root/patcher/programs', downloadProgramName);
                ...
                res.json({status: 404, message: 'Wrong username or password.'})
            }
        }
    }
});
